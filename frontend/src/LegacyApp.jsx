...
const DEFAULT_CONFIG = {
    ...
    viewer: {
        ...
        /** 看图模式亮度滑块上限（%），可在设置中修改，范围建议 150～800 */
    ...
    /** 设置「配置」页中远程预设列表的默认索引 URL（可被本机设置覆盖）；可为 GitHub raw 的 JSON */
...
/** 与 DEFAULT_CONFIG 顶层键一致：导出/导入时只读写这些键，避免注入未知字段 */
...
/** 支持本工具导出格式，或裸的「类 config.json」对象 */
...
function App() {
    ...
    // 活跃分类定义：提前声明，后续 useCallback 依赖数组需要引用（避免 TDZ）
    ...
    // activeImageCategories/Colors/MultiSelect 已提前声明（见函数顶部），此处只保留派生值
...
function SettingsModal({ onClose, cocoImageCategoryDefsActive = false }) {
    ...
    const [updateCheck, setUpdateCheck] = useState(null); // null | { loading: true } | { error: true } | { data: object }
    ...
    const [presetListStatus, setPresetListStatus] = useState(null); // null | loading | error | ok | empty
...
function ExportModal({ images, imageClassifications, imageNotes, datasetData, imageCategories, imageCategoryColors, onClose }) {
    ...
    const [expSplitBySourceDir, setExpSplitBySourceDir] = useState(true);
    ...
    const [expPreviewData, setExpPreviewData] = useState(null);
    useEffect(() => {
    ...
    const getDirSplit = useCallback((img) => {
        const joined = `${img?.source_path || ''}/${img?.file_name || ''}`.replace(/\\/g, '/').toLowerCase();
        const parts = joined.split('/').filter(Boolean);
        let hit = null;
        for (let i = 0; i < parts.length; i++) {
            const seg = parts[i];
            if (seg === 'train') hit = 'train';
            else if (seg === 'valid' || seg === 'val' || seg === 'validation') hit = 'valid';
        }
        return hit || 'train';
    }, []);

    const buildExperimentPreview = useCallback(() => {
        const all = Array.isArray(images) ? images : [];
        if (!expSplitBySourceDir) {
            let work = [...all];
            if (expDropDupes) {
                const seen = new Set();
                work = work.filter((img) => {
                    const bn = ((img?.file_name || '').replace(/\\/g, '/').split('/').pop() || '').trim().toLowerCase();
                    if (!bn) return false;
            ...
            const n = work.length;
            if (n <= 1) return { train: n, valid: 0, total: n, mode: 'random' };
            if (expTrainRatio >= 1) return { train: n, valid: 0, total: n, mode: 'random' };
            let tr = Math.floor(n * expTrainRatio);
            if (tr <= 0) tr = 1;
            if (tr >= n) tr = n - 1;
            return { train: tr, valid: n - tr, total: n, mode: 'random' };
        }
        const trainRaw = [];
        const validRaw = [];
        all.forEach((img) => {
            const s = getDirSplit(img);
            if (s === 'valid') validRaw.push(img);
            else trainRaw.push(img);
        });
        if (!expDropDupes) {
            return { train: trainRaw.length, valid: validRaw.length, total: trainRaw.length + validRaw.length, mode: 'dir' };
        }
        const dedupeOne = (arr) => {
            const seen = new Set();
            let kept = 0;
        ...
        const tr = dedupeOne(trainRaw);
        const va = dedupeOne(validRaw);
        return { train: tr, valid: va, total: tr + va, mode: 'dir' };
    }, [images, expSplitBySourceDir, expDropDupes, expTrainRatio, getDirSplit]);
    ...
    const openExperimentExportPreview = () => {
        if (!datasetData?.dataset_id) {
            alert('请先加载数据集');
            return;
        }
        setExpPreviewData(buildExperimentPreview());
        setExpPreviewOpen(true);
    };

    const executeExperimentExport = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch('/api/export_experiment_dataset_stream', {
                ...
                body: JSON.stringify({
                    ...
                    split_by_source_dir: expSplitBySourceDir,
            ...
            msg += `\n划分方式：${st.split_by_source_dir === false ? '随机比例' : '按目录名（train/valid），未命中默认归入 train。'}`;
            if (dupN > 0) {
    ...
    }, [datasetData, exp, expZipName, expTrainRatio, expSeed, expDropDupes, expSplitBySourceDir, expOutputPath, onClose]);

    const dismissOverlayOrModal = () => {
        if (expPreviewOpen) {
            setExpPreviewOpen(false);
            return;
        }
    ...
    const defaultCat = catList[0] || '未分类';
    return (
        <div className="modal-overlay" onClick={dismissOverlayOrModal}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: exportTab === 'experiment' ? '580px' : undefined }}>
                <div className="modal-header">
                    ...
                    <button type="button" className="modal-close" onClick={dismissOverlayOrModal}>✕</button>
                </div>
                <div className="modal-body">
                    ...
                    <div className="export-experiment-panel">
                        ...
                        <div className="export-experiment-dup-box" style={{ marginTop: '8px' }}>
                            <div className="export-experiment-checkbox-title" style={{ marginBottom: '6px' }}>划分模式</div>
                            <label className="export-experiment-checkbox-row">
                                <input
                                    type="radio"
                                    name="expSplitMode"
                                    checked={!expSplitBySourceDir}
                                    onChange={() => setExpSplitBySourceDir(false)}
                                    disabled={exporting}
                                />
                                <span>随机划分（按比例）</span>
                            </label>
                            <label className="export-experiment-checkbox-row">
                                <input
                                    ...
                                    name="expSplitMode"
                                    checked={expSplitBySourceDir}
                                    onChange={() => setExpSplitBySourceDir(true)}
                                    disabled={exporting}
                                />
                                <span>按目录名划分（train/valid）</span>
                            </label>
                            <p className="export-experiment-hint">目录模式下：命中 train 归训练集，命中 valid/val 归验证集，未命中默认归训练集；跨 train/valid 的同名文件不会互相去重。</p>
                        </div>
                ...
                <div className="modal-footer">
                    <button type="button" className="btn btn-secondary" onClick={dismissOverlayOrModal} disabled={exporting}>取消</button>
                    {exportTab === 'category' ? (
                        <button type="button" className="btn btn-primary" onClick={handleExport} disabled={exporting || selectedCategories.size === 0}>
                            {exporting ? '导出中...' : exp.exportButtonText}
                        </button>
                    ) : (
                        <button type="button" className="btn btn-primary" onClick={openExperimentExportPreview} disabled={exporting || expPreviewOpen}>
                            {exporting ? '导出中...' : (exp.experimentExportButton || '导出实验包')}
                        </button>
                    )}
                </div>
            </div>
            {expPreviewOpen && expPreviewData && (
                <div
                    className="export-preview-backdrop"
                    role="presentation"
                    onClick={() => setExpPreviewOpen(false)}
                >
                    <div
                    ...
                    >
                        <div className="export-preview-header">
                            <h4 id="export-preview-title">确认导出实验包</h4>
                            <p className="export-preview-subtitle">以下为根据当前选项估算的数量，实际以后端处理结果为准。</p>
                        </div>
                        <div className="export-preview-body">
                            <div className="export-preview-stats">
                                <div className="export-preview-stat export-preview-stat--train">
                                    <div className="export-preview-stat-label">训练集</div>
                                    <div className="export-preview-stat-value">{expPreviewData.train}</div>
                                    <div className="export-preview-stat-unit">张</div>
                                </div>
                                <div className="export-preview-stat export-preview-stat--valid">
                                    <div className="export-preview-stat-label">验证集</div>
                                    <div className="export-preview-stat-value">{expPreviewData.valid}</div>
                                    <div className="export-preview-stat-unit">张</div>
                                </div>
                            </div>
                            <div className="export-preview-total">
                                合计 <strong>{expPreviewData.total}</strong> 张
                            </div>
                            <ul className="export-preview-list">
                                <li>
                                    {expPreviewData.mode === 'dir'
                                        ? '划分：按目录名（train / valid / val / validation），未命中路径段默认归入训练集。'
                                        : `划分：随机比例 ${expTrainRatio}（seed=${expSeed}）。`}
                                </li>
                                <li>
                                    {expDropDupes
                                        ? (expPreviewData.mode === 'dir'
                                            ? '重名：仅在各自 split 内去重；跨 train/valid 的同名文件会保留。'
                                            : '重名：全局按文件名去重。')
                                        : '重名：严格模式，不去重。'}
                                </li>
                            </ul>
                        ...
                        <div className="export-preview-footer">
                            <button type="button" className="btn btn-secondary" onClick={() => setExpPreviewOpen(false)}>
                                返回修改
                            </button>
                            <button type="button" className="btn btn-primary" onClick={() => { void executeExperimentExport(); }}>
                                开始导出
                            </button>
                        </div>
                    </div>
...
function ImageViewer({ image, images, datasetId, categories, imageClassifications, imageNotes, imageCategories, imageCategoryColors, imageCategoryMultiSelect, onUpdateImageCategories, onUpdateImageCategoryColors, visiblePredModels, onUpdateVisiblePredModels, onClose, onNavigate, onUpdateCategory, onUpdateCategories, onUpdateNote, onAnnotationsSaved }) {
    ...
    const [extraAnnCategories, setExtraAnnCategories] = useState([]); // 看图内临时新增的目标框类别
    // 十字辅助线 refs
    ...
    const availableAnnCategories = useMemo(() => {
        const out = [];
        const seen = new Set();
        const push = (v) => {
        ...
        (extraAnnCategories || []).forEach(push);
        (localAnns || []).forEach(a => push(a && a.category));
        if (out.length === 0) push('目标');
        return out;
    }, [categories, extraAnnCategories, localAnns]);
        ...
        <div className={`viewer-modal${annotateMode ? ' viewer-modal--annotate' : ''}`} ref={viewerModalRef}>
            <div className="viewer-header">
                <div className="viewer-nav">
                    <button className="viewer-nav-btn" title="上一张（← / A）" disabled={imageIdx <= 0} onClick={() => navigate(-1)}>‹</button>
                    ...
                    <button className="viewer-nav-btn" title="下一张（→ / D）" disabled={imageIdx >= images.length - 1} onClick={() => navigate(1)}>›</button>
                </div>
                ...
                <div className="viewer-controls">
                    ...
                    <button type="button" className="viewer-nav-btn" title="适应窗口（0）" onClick={() => { setZoom(fitZoom()); setPanX(0); setPanY(0); }}>⊡</button>
                            </div>
                            <div className="viewer-toolbar-group">
            ...
            <div className={`viewer-body ${hasPredAnnotations ? 'has-left-pred' : ''} ${sidebarDock === 'right' ? 'dock-right' : 'dock-left'}`}>
                ...
                <div className="viewer-sidebar viewer-sidebar-main viewer-sidebar-left viewer-slot-main" style={{ width: mainSidebarWidth, flexShrink: 0, minWidth: 0 }}>
                    ...
                    {/* 多选批量改类别 */}
                    {annotateMode && selectedAnnIdxSet.size > 1 && availableAnnCategories.length > 0 && (
                        <div style={{padding:'6px 10px', borderBottom:'1px solid var(--border)', background:'var(--bg-raised)'}}>
                            <div style={{fontSize:'11px', color:'#faad14', marginBottom:'5px'}}>批量改类别 ({selectedAnnIdxSet.size} 个框)</div>
                            <div style={{display:'flex', flexWrap:'wrap', gap:'4px'}}>
                                {availableAnnCategories.map(cat => (
                                        ...
                                        {isSelected && (
                                            <div style={{ padding: '4px 8px 6px', display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        const name = (window.prompt('新增目标框类别名') || '').trim();
                                                        if (!name) return;
                                                        setExtraAnnCategories(prev => prev.includes(name) ? prev : [...prev, name]);
                                                        commitChange(localAnns.map((a, i) => i === idx ? { ...a, category: name } : a));
                                                        lastCategoryRef.current = name;
                                                    }}
                                                    style={{
                                                        padding: '2px 8px', fontSize: 'var(--font-xs)', borderRadius: '4px', cursor: 'pointer',
                                                        border: '1px dashed var(--accent)', background: 'rgba(95,140,255,0.08)', color: 'var(--accent)',
                                                    }}
                                                >+ 新增类别</button>
                                            </div>
                                        )}
                ...
                >
                    ...
                    {annotateMode && categoryPickerBbox && (
                        <CategoryPicker
                            categories={availableAnnCategories}
